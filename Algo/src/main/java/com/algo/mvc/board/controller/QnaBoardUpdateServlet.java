...
@WebServlet(name = "qnaBoardUpdate", urlPatterns = { "/qnaBoard/update" })
...
//				board.setSmallCategoryId(mr.getParameter("category"));
				...
				if (result > 0 ) {
					...
					request.setAttribute("location", "/qnaBoard/qnaPost?no=" + board.getPostNo());
				} else {
					// 게시글 수정 실패
					request.setAttribute("msg", "게시글 수정 실패");
					request.setAttribute("location", "/qnaBoard/update?no=" + board.getPostNo());
				}
